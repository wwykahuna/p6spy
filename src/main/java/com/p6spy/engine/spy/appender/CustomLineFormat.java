...
public class CustomLineFormat implements MessageFormattingStrategy {
	 ...
	 * @param elapsed      the time in milliseconds that the operation took to
	 *                     complete
	 * @param category     the category of the operation
	 * @param prepared     the SQL statement with all bind variables replaced with
	 *                     actual values
	...
	public String formatMessage(final int connectionId, final String now, final long elapsed, final String category,
		...
		return customLogMessageFormat.replaceAll(Pattern.quote(CONNECTION_ID), Integer.toString(connectionId))
				...
				.replaceAll(Pattern.quote(EFFECTIVE_SQL_SINGLELINE),
						Matcher.quoteReplacement(P6Util.singleLine(prepared)))
