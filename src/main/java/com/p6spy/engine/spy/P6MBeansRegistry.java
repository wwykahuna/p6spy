...
public class P6MBeansRegistry {
	...
	public void registerMBeans(Collection<P6LoadableOptions> allOptions) throws MBeanRegistrationException,
			InstanceNotFoundException, MalformedObjectNameException, NotCompliantMBeanException {
		boolean jmx = true;
		String jmxPrefix = "";
		...
		for (P6LoadableOptions options : allOptions) {
			try {
				...
				// sounds like someone registered beans already (before we had a chance to do
				// so)
	...
	public void unregisterAllMBeans(String jmxPrefix) throws MBeanRegistrationException, MalformedObjectNameException {
		...
		// to prevent naming conflicts: let's unreg also possible leftovers (with the
		// same prefix)
		for (ObjectName objectName : mbs.queryNames(new ObjectName(getPackageName(jmxPrefix) + ":name=com.p6spy.*"),
				null)) {
			try {
