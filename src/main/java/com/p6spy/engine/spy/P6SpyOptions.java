...
public class P6SpyOptions extends StandardMBean implements P6SpyLoadableOptions {
	...
	static {
		...
		defaults.put(CUSTOM_LOG_MESSAGE_FORMAT,
				String.format("%s|%s|%s|connection%s|%s", CustomLineFormat.CURRENT_TIME,
						CustomLineFormat.EXECUTION_TIME, CustomLineFormat.CATEGORY, CustomLineFormat.CONNECTION_ID,
						CustomLineFormat.SQL_SINGLE_LINE));
	 ...
	 *
	 * @param databaseDialectTimestampFormat The databaseDialectTimestampFormat to
	 *                                       set
	 */
	@Override
