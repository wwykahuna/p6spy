...
public class P6DataSource implements DataSource, ConnectionPoolDataSource, XADataSource, Referenceable, Serializable {
	...
	/**
	 * Create a P6Spy DataSource wrapping another DataSource. This constructor is
	 * primarily used by dependency injection frameworks.
	 *
	...
	protected synchronized void bindDataSource() throws SQLException {
		// we'll check in the synchronized section again (to prevent unnecessary
		// reinitialization)
		...
		try {
			...
			throw new SQLException("P6DataSource: naming exception during jndi lookup of Real Data Source Name of '"
					+ rdsName + "'. " + e.getMessage(), e);
		}
	...
	private void setDataSourceProperties(Map<String, String> props) throws SQLException {
					...
					if (key.toLowerCase().equals(propertyName)) {
						try {
							...
							if (paramType.isAssignableFrom(String.class)) {
								...
								P6LogQuery.debug(
										"calling " + methodName + " on DataSource " + rdsName + " with " + value);
								...
								P6LogQuery.debug("method " + methodName + " on DataSource " + rdsName
							...
							throw new SQLException("spy.properties file includes" + " datasource property " + key
									+ " for datasource " + rdsName + " but access is denied to method " + methodName,
									e);
						} catch (java.lang.reflect.InvocationTargetException e) {
							throw new SQLException("spy.properties file includes" + " datasource property " + key
									+ " for datasource " + rdsName + " but call method " + methodName + " fails", e);
						}
					}
				}
			}
		...
		// log properties defined in spy.properties that were not found on the data
		// source.
		for (String key : props.keySet()) {
			if (!matchedProps.containsKey(key)) {
				P6LogQuery.debug("spy.properties file includes" + " datasource property " + key + " for datasource "
						+ rdsName + " but class " + klass.getName() + " has no method" + " by that name");
			}
		}
	}
